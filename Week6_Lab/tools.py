...
Each function is a callable tool the AI agent can invoke.
Tools cover: Snowflake querying, RAG retrieval, analytics/stats, and text summarization.
"""
...
def query_snowflake(sql: str) -> dict[str, Any]:
    ...
    Only SELECT statements are permitted; DML/DDL is rejected to keep the
    tool safe for agent use.

    Args:
        sql: A valid Snowflake SQL SELECT statement.

    Returns:
        dict with keys:
            - columns (list[str]): column names
            - rows (list[list]): result rows (up to 200)
...
def list_tables() -> dict[str, Any]:
    """
    List all tables available in the current Snowflake database and schema.
