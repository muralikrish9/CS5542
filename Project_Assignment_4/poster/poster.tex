...
\author{Murali Ediga \quad \textit{Advisor: Dr.\ Prasun Chatterjee}}
\institute[shortinst]{University of Missouri}
...
\begin{column}{\colwidth}
  ...
  \begin{alertblock}{Problem Statement / Motivation}
    ...
    Frontier LLMs now autonomously hack systems --- running \texttt{nmap},
    exploiting CVEs, and pivoting laterally \textbf{without human intervention}.
    Defenders see only shell commands; the driving LLM is hidden behind the
    agent scaffold.

    \begin{itemize}
      \item Each LLM family has \textbf{different vulnerabilities} to prompt
            injection --- but defenders don't know which model they're facing
      \item Without fingerprinting, defenders must deploy
            \textbf{one-size-fits-all} countermeasures
      \item No existing work fingerprints LLMs from \textbf{terminal commands
            in an agentic setting}
    \end{itemize}

    \heading{Goal}

    Identify which LLM family is driving an agent session from terminal
    behavior alone, then deploy \textbf{targeted} defenses.
  ...
  \begin{block}{System Architecture --- Live Deployment Pipeline}
    ...
    \begin{tikzpicture}[
        ...
        box/.style={rectangle, rounded corners=3pt, draw=black!50, fill=#1,
                    line width=0.8pt, minimum width=13cm, minimum height=1.4cm,
                    align=center, font=\normalsize},
        arrow/.style={->, line width=1.2pt, color=black!50},
        lbl/.style={font=\footnotesize\itshape, text=black!60},
      ]
      \node[box=lightpurple] (agent)
        {\textbf{AI Agent Session} --- unknown model family};
      \node[box=lightorange, below=of agent] (hp)
        {\textbf{Cowrie Honeypot (GCP VM)} --- captures raw commands};
      \node[box=lightblue, below=of hp] (agg)
        {\textbf{Session Aggregator} --- 2-min sliding window};
      \node[box=wingreen, below=of agg] (clf)
        {\textbf{TF-IDF + LinearSVC Classifier} --- predict family + confidence};
      \node[box=losered, below=of clf] (dpi)
        {\textbf{Fingerprint-Guided DPI Router} --- deploy best payload};
    ...
    Deployed on GCP. Live demo achieved \textbf{87\% confidence} on incoming
    agent sessions. Claude flagged as resistant (0\% ASR) --- no payload deployed.
  ...
  \begin{block}{Results \& Evaluation}
    ...
    \begin{table}
      ...
      \begin{tabular}{l c l}
        ...
        \midrule
        Timing (control) & 92\% & No effect on TF-IDF \\
        Obfuscation & 71\% & Partial degradation \\
        Mimicry & 67\% & Does \textbf{not redirect} \\
        \midrule
        \textbf{Vulnetic blind} & \textbf{78\%} & Unknown commercial scaffold \\
        \bottomrule
      \end{tabular}
    \end{table}
...
\begin{column}{\colwidth}
  ...
  \begin{block}{Application: Targeted Defensive Prompt Injection}

    \textbf{The killer app of fingerprinting:} once you know the model, you
    can exploit its specific weaknesses.

    Three DPI payloads planted in CTF environment files:
    ...
    Know the model $\rightarrow$ choose the right weapon.
  ...
  \begin{alertblock}{Conclusion \& Future Work}
    ...
    \begin{enumerate}
      ...
      \item \textbf{Scaffold-invariant} (LOSO F1 = 0.812), evasion-resistant
      \item \textbf{+26.5pp targeting lift} via fingerprint-guided DPI
  ...
  \begin{block}{References}
    ...
    \footnotesize
    \begin{enumerate}
      \item Pasquini et al. ``LLMmap: Fingerprinting For LLMs.'' arXiv:2407.15847, 2024.
      \item Wang et al. ``CHeaT: A Chatbot Detection Tool.'' WWW, 2024.
